...
Page({
  ...
  onLoad: function (options) {
    ...
    const activeDeals = wx.getStorageSync('active_deals') || [];
    let deal = activeDeals.find(d => d.id === id || String(d.id) === String(id));
    ...
    if (!deal) {
      const deals = [
        ...
        },
        {
          ...
          description: 'Pneus de quality premium aux prix avantageux :'
        },
        {
          id: 5,
      ...
      deal = deals.find(d => d.id === id) || deals[0];
    }

    if (!deal.description) {
      deal.description = `Profitez de cette offre exceptionnelle chez notre partenaire ${deal.brandName || 'Tiqqa Partner'}.`;
    }
